...
void _weave_calculate_mutual_information_slow(float *anomaly, int n_samples,
    ...
    for (i = 0; i < N; i++) {
        for (k = 0; k < n_samples; k++) {
            ...
            rescaled = scaling * (anomaly[i*N+k] - range_min);

            if (rescaled < 1.0) {
                symbolic[i*N+k] = rescaled * n_bins;
            }
            else {
                symbolic[i*N+k] = n_bins - 1;
            }

            //  Calculate 1d-histograms for single time series
            symbol = symbolic[i*N+k];
            hist[i*N+symbol] += 1;
        }
    }
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j <= i; j++) {
            ...
            if (i != j) {
                ...
                //  (i,j).
                for (k = 0; k < n_samples; k++) {
                    symbol_i = symbolic[i*N+k];
                    symbol_j = symbolic[j*N+k];
...
void _weave_calculate_mutual_information_fast(float *anomaly, int n_samples,
    int N, int n_bins, float scaling, float range_min, long *symbolic,
    long *hist, long *hist2d, float *mi) {
